...
def get_general_game_information(soup):
    ...
    return game_title, game_diff, quality_index, team_count, game_type, forum_resonance, date, authors
...
class Analyzer(threading.Thread):
    ...
    def analyze(self, url):
        print(url)
        res = session.get('http://vbratske.en.cx' + url, headers=headers)
        soup = BeautifulSoup(res.text, 'html.parser')

        game_title, game_diff, quality_index, team_count, game_type, forum_resonance, date, authors = get_general_game_information(
            soup)
        # если игра состоялась и индекс качества есть
        try:
            game = Game.objects.get_or_create(name=game_title.text,
                                              url='http://vbratske.en.cx' + url,
                ...
                if game_type == 'Командная':
                    ...
                    try:
                        ...
                        for element in monitoring:
                            print(element)
                            player = \
                            Player.objects.get_or_create(name=element[0].text, url=MAIN_URL + element[0]['href'])[0]
                            print(player)
                            if element[1] == 'w':
                                correct = False
                            else:
...
if __name__ == "__main__":
    ...
    game_list = []
    for i in range(1, 9):
        print('Сбор url, страница {}'.format(i))
